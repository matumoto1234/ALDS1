...
int main() {
  ...
  for (int c = 'a'; c <= 'z'; ++c) {
    ...
    int count = counter[idx];
    pq.emplace(count, idx);
  }


  constexpr int max_num_of_nodes = 26 + (26 - 1) + 1;
  if (pq.size() == 1) {
    pq.emplace(0, max_num_of_nodes - /*to 0-indexed*/ 1);
  }

  int idx = 'z' - 'a';
  vector<vector<int>> G(max_num_of_nodes);

  while (len(pq) >= 2) {
    auto [a_cnt, a_idx] = pq.top();
    pq.pop();

    auto [b_cnt, b_idx] = pq.top();
    pq.pop();

    idx++;
    G[idx].emplace_back(a_idx);
    G[idx].emplace_back(b_idx);
    pq.emplace(a_cnt + b_cnt, idx);
  }

  vector<int> depths(max_num_of_nodes);
  depths[idx] = 0;

  auto dfs = [&](auto &&f, int v, int pre) -> void {
